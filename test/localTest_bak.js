...
async function main() {
  ...
  const manager = await RealEstateManager.deploy(
    ...
    ethers.parseUnits("1000000", 6), // _propertyValue
    "QmRYfrZ4yobkBMUD3Y4d4NV28ZeTUFCBwWSm1Z3B2Ndkca", // _ipfsHash
  ...
  const token = await RealEstateToken.deploy(
    "Real Estate Token",
    "RET",
    owner.address,  // Token的所有者
    managerAddress  // Manager合约地址，将被设置为特殊操作者
  );
  ...
  // 设置Manager的代币合约
  await manager.setTokenContract(tokenAddress);
  console.log("已设置Token合约到Manager");

  // 获取Manager中的Token地址
  const managerTokenAddress = await manager.getPropertyToken();
  console.log("Manager使用的Token地址:", managerTokenAddress);
...
async function runTests() {
  ...
  // 获取合约当前USDT余额
  const contractBalanceBefore = await usdt.balanceOf(await manager.getAddress());
  console.log("合约赎回前余额:", ethers.formatUnits(contractBalanceBefore, 6));
...
//   console.log("用户2赎回金额:", ethers.formatUnits(redeemedAmount, 6), "USDT");
//   console.log("赎回后 USDT 余额:", ethers.formatUnits(finalUSDTBalance, 6));
  
//   // 验证合约余额减少
//   const contractBalanceAfter = await usdt.balanceOf(await manager.getAddress());
//   console.log("合约赎回后余额:", ethers.formatUnits(contractBalanceAfter, 6));

  // 7. 赎回功能测试
  // 7. 赎回功能测试 - 传递必要的参数
  const redeemResult = await testRedeemShares(token, manager,usdt, user2, "用户2");
...
async function testRedeemShares(token, manager, usdt, user, userLabel) {
  console.log(`\n[测试] ${userLabel}赎回份额...`);
  ...
  // 获取用户要赎回的份额
  const sharesToRedeem = await token.balanceOf(user.address);
  console.log(`${userLabel}要赎回的份额:`, ethers.formatUnits(sharesToRedeem, 18), "份额");
  
  // 获取用户赎回前的USDT余额
  const initialUSDTBalance = await usdt.balanceOf(user.address);
  console.log(`${userLabel}赎回前 USDT 余额:`, ethers.formatUnits(initialUSDTBalance, 6), "USDT");
  ...
  // 执行赎回
  console.log("执行赎回...");
  await manager.connect(user).redeemShares(sharesToRedeem);
  
  // 获取赎回后数据
  const finalUSDTBalance = await usdt.balanceOf(user.address);
  const redeemedAmount = finalUSDTBalance - initialUSDTBalance;
  console.log(`${userLabel}实际赎回金额:`, ethers.formatUnits(redeemedAmount, 6), "USDT");
  console.log(`赎回后 ${userLabel} USDT 余额:`, ethers.formatUnits(finalUSDTBalance, 6), "USDT");
  
  // 验证差异在可接受范围内
  const difference = redeemedAmount > expectedRedeemAmount 
    ? redeemedAmount - expectedRedeemAmount
    : expectedRedeemAmount - redeemedAmount;
  
  console.log("赎回金额差异:", ethers.formatUnits(difference, 6), "USDT");
